...
func TestCheckSpiceHandler_Success(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			queryParams:    "id=1&name=クミン&alias=クミン&taste=辛味&flavor=辛味&family=セリ科&origin=インド",
			expectedStatus: http.StatusOK,
		...
		},
		{
			...
			mockDB: &MockDB{
				data: []db.Spices{
					{Id: 1, Name: "クミン1", Alias: "クミン1", Taste: "辛味", Flavor: "辛味", Family: "セリ科", Origin: "インド"},
					{Id: 2, Name: "クミン2", Alias: "クミン2", Taste: "辛味", Flavor: "辛味", Family: "セリ科", Origin: "インド"},
				},
				err: nil,
			},
			queryParams:    "id=2&name=クミン2&alias=クミン2&taste=辛味&flavor=辛味&family=セリ科&origin=インド",
			expectedStatus: http.StatusOK,
			expectedBody:   `{"match":true}`,
		},
		{
			...
			queryParams:    "id=1&name=コリアンダー&alias=クミン&taste=辛味&flavor=辛味&family=セリ科&origin=インド",
			expectedStatus: http.StatusOK,
...
func TestCheckSpiceHandler_Failed(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			queryParams:    "id=abc&name=クミン&alias=クミン&taste=辛味&flavor=辛味&family=セリ科&origin=インド",
			expectedStatus: http.StatusBadRequest,
		...
		},
		{
			...
			queryParams:    "id=999&name=クミン&alias=クミン&taste=辛味&flavor=辛味&family=セリ科&origin=インド",
